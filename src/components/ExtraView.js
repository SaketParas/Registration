...
class ExtraView extends Component {
    ...
    render() {
        ...
        var button = pageNumbers.map(btn => {
            return(
            <button  type="button" class="btn btn-info" onClick={() =>this.handleButton(btn)}>{btn}</button>
            )
        })
        ...
        var nextButton = () => {
            if (this.state.page !== this.state.total.length) {
                return (
                    <button className="btn btn-success ml-3" onClick={() => this.handleNext(this.state.page)}> Next</button>
                )
          
            }
            else {
                return (
                    <button className="btn btn-warning" disabled>Next</button>
                )
            }
          }
          ...
          var prevButton = () => {
            if (this.state.page !== 1) {
                ...
                return (
                    <button className="btn btn-warning "  disabled>Prev</button>
                )
            }
          }
        ...
        return (
            <React.Fragment>
                    ...
                    <select className="form-control offset-2  btn btn-success"style={{width:"120px"}} onChange={this.handleDropDown} name="perPage">
                                 ...
                                 <div className="row">
                                     <div className="col-6">
                        <h3 class="text-info mt-5">View My Comments</h3>
                        {show}
                    </div>
                </div>
